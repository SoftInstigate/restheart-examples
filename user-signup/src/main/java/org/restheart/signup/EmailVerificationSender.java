...
public class EmailVerificationSender implements MongoInterceptor {
    ...
    public void init(Map<String, Object> confs) {
        from = argValue(confs, "from");
        fromName = argValue(confs, "from-name");
        smptUsername = argValue(confs, "smtp-username");
        smtpPassword = argValue(confs, "smtp-password");
        host = argValue(confs, "host");
        port = argValue(confs, "port");
        verifierSrvUrl = argValue(confs, "verifier-srv-url");
    }

    @Override
    public void handle(MongoRequest request, MongoResponse response) throws Exception {
        var to = response.getDbOperationResult().getNewId()
                .asString().getValue();

        var vcode = response.getDbOperationResult().getNewData()
                .get("code").asString().getValue();
    ...
    public boolean resolve(MongoRequest request, MongoResponse response) {
        return response.getDbOperationResult() != null
    ...
    private void send(String to, String vcode) throws Exception {
        ...
        var body = BODY_TEMPLATE.replace("${LINK}",
                verifierSrvUrl
                        .concat("?code=").concat(vcode)
                        .concat("&username=").concat(to)
        );
